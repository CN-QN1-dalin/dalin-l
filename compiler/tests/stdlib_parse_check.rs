    ...
    for entry in std::fs::read_dir(&base).unwrap().filter_map(|e| e.ok()) {
        ...
        if path.extension().map_or(false, |ext| ext == "dal") {
            ...
            let tokens = match lex.tokenize() {
                Ok(t) => t,
                Err(e) => { errors += 1; failures.push((path.display().to_string(), 0, format!("LEX: {e}"))); continue; }
            };
            ...
            match parser.parse() {
                Ok((_, errs)) => {
                    if !errs.is_empty() {
                        errors += 1;
                        let first = &errs[0];
                        failures.push((path.display().to_string(), errs.len(), format!("L{}C{}: {}", first.line, first.column, first.message)));
                    }
                }
                Err(e) => { errors += 1; failures.push((path.display().to_string(), 1, format!("FATAL: {e}"))); }
            }
        }
    }

...
mod scheduler_tests {
    use crate::scheduler::{Capability, CapabilityScheduler, Node};
    ...
    fn cpu_task_can_go_to_any_node() {
        let s = CapabilityScheduler::new(vec![
    ...
    fn gpu_task_cannot_go_to_cpu_only() {
        let s = CapabilityScheduler::new(vec![make_node("cpu-only", &[Capability::Cpu])]);
        assert!(s.place(&Capability::Gpu).is_none(), "仅 Cpu 节点不能调度 Gpu");
    }

    #[test]
    fn net_capable_node_cover_all_capabilities() {
        let s = CapabilityScheduler::new(vec![make_node(
            "net-full",
    ...
    fn sfa_task_requires_sfa_or_higher() {
        let s = CapabilityScheduler::new(vec![make_node("gpu-node", &[Capability::Cpu, Capability::Gpu])]);
        assert!(s.place(&Capability::Sfa).is_none(), "Gpu 节点不能执行 Sfa 任务");
        let s2 = CapabilityScheduler::new(vec![make_node("sfa-node", &[Capability::Sfa])]);
    ...
    fn breaker_opens_after_threshold_failures() {
        let s = CapabilityScheduler::new(vec![make_node("failing", &[Capability::Cpu, Capability::Gpu]).with_quota(100)]);
    ...
    fn success_resets_breaker() {
        let s = CapabilityScheduler::new(vec![make_node("unstable", &[Capability::Cpu]).with_quota(100)]);
    ...
    fn quota_exhaustion_triggers_backpressure() {
        let s = CapabilityScheduler::new(vec![make_node("limited", &[Capability::Cpu]).with_quota(2)]);
        assert!(s.place(&Capability::Cpu).is_some());
        assert!(s.place(&Capability::Cpu).is_some());
    ...
    fn release_frees_capacity() {
        let s = CapabilityScheduler::new(vec![make_node("pool", &[Capability::Cpu]).with_quota(2)]);
    ...
    fn load_counter_tracks_placements() {
        let s = CapabilityScheduler::new(vec![make_node("counter", &[Capability::Cpu]).with_quota(100)]);
    ...
    fn add_node_dynamically() {
        let mut sched = CapabilityScheduler::new(vec![make_node("a", &[Capability::Cpu]).with_quota(10)]);
    ...
    fn sync_replaces_nodes() {
        let mut sched = CapabilityScheduler::new(vec![make_node("old", &[Capability::Cpu]).with_quota(10)]);
    ...
    fn place_by_spec_parses_gpu() {
        let s = CapabilityScheduler::new(vec![make_node("gpu-node", &[Capability::Gpu]).with_quota(10)]);
    ...
    fn place_by_spec_unknown_defaults_to_cpu() {
        let s = CapabilityScheduler::new(vec![make_node("cpu-only", &[Capability::Cpu]).with_quota(10)]);
...
mod store_tests {
    ...
    async fn status_transitions() {
        ...
        store.set_status(&rec.id, TaskStatus::Scheduled).await;
        store.set_status(&rec.id, TaskStatus::Running).await;
        store.set_status(&rec.id, TaskStatus::Succeeded).await;
        let final_ = store.get(&rec.id).await.unwrap();
        assert_eq!(final_.status, TaskStatus::Succeeded);
    }
    ...
    async fn cancel_returns_bool() {
        ...
        assert_eq!(store.get(&rec.id).await.unwrap().status, TaskStatus::Canceled);
    }
    ...
    async fn event_subscriptions_receive_status_changed() {
        ...
        while let Ok(ev) = rx.try_recv() {
            if let TaskEvent::StatusChanged(r) = ev {
...
mod factory_tests {
    #[test]
    fn backend_name_labels_correctly() {
        assert_eq!(crate::store_factory::backend_name(""), "memory");
        assert_eq!(crate::store_factory::backend_name("memory://"), "memory");
        assert_eq!(crate::store_factory::backend_name("redis://localhost:6379"), "redis");
        assert_eq!(crate::store_factory::backend_name("postgres://u@h/d"), "postgres");
        assert_eq!(crate::store_factory::backend_name("etcd://127.0.0.1:2379"), "etcd");
        assert_eq!(crate::store_factory::backend_name("s3://unknown"), "unknown");
    }

    #[tokio::test]
    async fn factory_memory_works() {
        let s = crate::store_factory::build_task_store("", "ft").await;
        assert!(s.is_ok());
        assert!(crate::store_factory::build_task_store("memory://explicit", "ft2").await.is_ok());
    }
}
...
mod transport_tests {
    use crate::dispatch::{DispatchBroker, InMemoryDispatchBroker, DispatchTask};
    use crate::registry::{self, TaskRecord, TaskStatus};
    use crate::transport::{EventBus, InMemoryEventBus};

    fn sample_event() -> registry::TaskEvent {
        registry::TaskEvent::Submitted(TaskRecord {
            id: "bus-t1".into(),
            name: "work".into(),
            parent: None,
            effect: "spawn".into(),

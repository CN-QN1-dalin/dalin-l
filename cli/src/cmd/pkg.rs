...
use dalin_compiler::package::{
    DependencyEntry, DependencyGraph, DependencySource, LockEntry, Lockfile, PackageInfo,
    PackageManager, PackageManifest, SemVer, parse_package_manifest,
};
...
fn cmd_build(_args: &HashMap<String, String>) -> Result<(), String> {
    ...
    // 选择 registry 后端: 默认 HTTPS registry，可用 DALIN_REGISTRY 覆盖为
    // file:// 路径 (私有/离线 registry) 或自定义 https 地址。
    let registry_url =
        std::env::var("DALIN_REGISTRY").unwrap_or_else(|_| "https://registry.dal.in".to_string());

    let mut pm = PackageManager::new(".dalan/cache".to_string(), registry_url.clone());
    ...
    for (name, ver) in &resolved {
        match pm.get_package(name, ver) {
            ...
            Err(e) => {
                // 优雅降级: registry 不可达时仍写入 manifest-only 条目
                println!("  Warning: could not fetch {}: {}", name, e);
                entries.push(LockEntry {
                    name: name.clone(),
                    version: ver.display(),
                    checksum: None,
    ...
    if any_real {
        println!(
            "  Resolved {} dependencies (verified, with SHA-256 checksums)",
            resolved.len()
        );
    } else if !resolved.is_empty() {
        println!(
            "  Resolved {} dependencies (manifest-only; registry unreachable — checksums pending)",
            resolved.len()
        );
    } else {
